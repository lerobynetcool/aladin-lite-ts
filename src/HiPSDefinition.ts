...
export type HipsServiceDesc = {
	...
	"hips_service_url_2" ?: string,
	"hips_service_url_3" ?: string,
	"hips_service_url_4" ?: string,
}
let AL_CACHE_CLASS_LEVEL: HipsServiceDesc[] = [{
...
let listHipsProperties: HipsServiceDesc[] = [] // this variable stores our current knowledge
...
function merge(baseList: HipsServiceDesc[], newList: HipsServiceDesc[]): HipsServiceDesc[] {
	let newListById:{[key: string]: HipsServiceDesc} = {}
	newList.forEach(item => newListById[item.ID] = item)
	return baseList.map((item: any) => {
		let id = item.ID
		if (newListById.hasOwnProperty(id)) {
			let itemToAdd: any = newListById[id]
...
export class HiPSDefinition {
	...
	// getServiceURLs(httpsOnly: boolean) {
	...
	static getLocalStorageDefinitions(): any[] {
		try {
	...
	static storeInLocalStorage(properties: any) {
		try {
	...
	static getRemoteDefinitions(params: any, successCallbackFn = (data: any)=>{}, failureCallbackFn = ()=>{}) {
	...
	static getALDefaultHiPSDefinitions() {
		return listHipsProperties
			...
			.filter( propt => (propt.client_application as string|string[])?.indexOf('AladinLite')>=0 )
	...
	static parseHiPSProperties(propertiesStr: string) {
		if (propertiesStr==null) return null

		let propertiesDict: {[key:string]: any} = {}
		propertiesStr
			.replace(/[\r]/g, '')             // remove CR characters
			.split('\n')                      // split on LF
			.map( line => line.trim() )       // remove whitespace at beginning and end
	...
	static findByID(id: string, callback = (candidates: HiPSDefinition[])=>{}) {
		// look first locally
	...
	static findByIDLocal(id2search: string): Array<HiPSDefinition> {
		return listHipsProperties
	...
	static findByIDRemote(id: string, callback: (a: HiPSDefinition[]) => void) {
		HiPSDefinition.findHiPSRemote({ID: `*${id}*`}, callback)
	}

	// search a HiPS according to some criteria
	static findHiPSRemote(searchOptions: any = {}, callback = (a: Array<HiPSDefinition>)=>{}) {
		...
		HiPSDefinition.getRemoteDefinitions(searchOptions, (candidates) => {
			callback(candidates.map( (candidate: any) => new HiPSDefinition(candidate) ))
		})
	}
	...
	static fromURL(url: string, callback = (h?: HiPSDefinition)=>{}) {
		let hipsUrl: string
		let propertiesUrl: string
		...
		let callbackWhenPropertiesLoaded = (properties: any) => {
			...
			let hipsPropertiesDict = HiPSDefinition.parseHiPSProperties(properties) as any // TODO : could be null
	...
	static fromProperties(properties: any) { return new HiPSDefinition(properties) }
