...
// REPRENDRE LA,  EN CREANT l'OBJET HiPSDefinition ou FAIRE dans l'autre sens
			...
			// remove final slash
			if (rootUrl.slice(-1) === '/') this.rootUrl = rootUrl.substr(0, rootUrl.length-1);
	...
	HpxImageSurvey.prototype.init = function(view, callback) {
		...
		if (!this.cm) this.cm = new ColorMap(this.view);
...
//////////////////////////////
	...
	HpxImageSurvey.prototype.drawAllsky = function(ctx, cornersXYViewMap, norder, view) {
		...
		if (this.view.curNorder>6) return;

		if ( ! this.allskyTexture || !Tile.isImageOk(this.allskyTexture) ) return;
