...
View = (function() {
	...
	function View (aladin, location, fovDiv, cooFrame, zoom) {
			...
			if (cooFrame) this.cooFrame = cooFrame;
			else this.cooFrame = CooFrameEnum.GAL;
	...
	View.prototype.setCursor = function(cursor) {
		if (this.reticleCanvas.style.cursor==cursor) return;
		if (this.mode==View.TOOL_SIMBAD_POINTER) return;
	...
	updateFovDiv = function(view) {
		...
		// update FoV value
		var fovStr;
		if (view.fov>1)         fovStr = Math.round(view.fov*100)/100 + "°";
		else if (view.fov*60>1) fovStr = Math.round(view.fov*60*100)/100 + "'";
		else                    fovStr = Math.round(view.fov*3600*100)/100 + '"';
	...
	createListeners = function(view) {
		...
		if ('ontouchstart' in window) hasTouchEvents = true;
		...
		onDblClick = function(e) {
			...
			if (view.cooFrame.system==CooFrameEnum.SYSTEMS.GAL) radec = CooConversion.GalacticToJ2000([lonlat.ra, lonlat.dec]);
			else radec = [lonlat.ra, lonlat.dec];

			view.pointTo(radec[0], radec[1]);
		};
		if (! hasTouchEvents) $(view.reticleCanvas).dblclick(onDblClick);
		...
		$(view.reticleCanvas).bind("mousedown touchstart", function(e) {
			...
			view.dragging = true;
			if (view.mode==View.PAN) view.setCursor('move');
			else if (view.mode==View.SELECT) view.selectStartCoo = {x: view.dragx, y: view.dragy};
